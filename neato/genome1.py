...
class genome(object):
    ...
    def createNetwork(self):
        for i in range(self._inputs):
            # self.nodes.append(node(self.total_nodes, self.input_layer, self.default_activation))
            self._nodes[i] = node(self.total_nodes, self.input_layer, self.default_activation)
            self.total_nodes += 1
            # print(self.total_nodes)
        for i in range(self._inputs, self._inputs+self._outputs):
            # self.nodes.append(node(self.total_nodes, self.output_layer, self.default_activation))
            self._nodes[i] = node(self.total_nodes, self.output_layer, self.default_activation)
            self.total_nodes += 1
    ...
    def exists(self, nn):
        ...
        for (i,j) in self._connections:
            if self._connections[(i,j)].innov == nn:
                return True
        return False
    ...
    def add_node(self):
        ...
        self._nodes[new_node] = node(new_node,new_node_layer,self.default_activation)
    ...
    def random_pair(self):
        ...
        if self.total_nodes == self._inputs+self._outputs:
            i = random.choice([n for n in range(self.total_nodes) if not self.is_output(n)])
            j_list = [n for n in range(self.total_nodes) if not self.is_input(n) and n != i and self._nodes[n].layer > self._nodes[i].layer]
